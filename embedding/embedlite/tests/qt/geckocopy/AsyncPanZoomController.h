 ...
 * per frame.
 */
class AsyncPanZoomController MOZ_FINAL : public EmbedKineticListener {
...
public:
  ...
  AsyncPanZoomController(mozilla::embedlite::EmbedLiteApp*, mozilla::embedlite::EmbedLiteView*,
                         GeckoContentController* aController = nullptr,
   ...
   * composite.
   */
  bool SampleContentTransformForFrame(const QDateTime& aSampleTime,
  ...
  float GetTouchStartTolerance() { return 1.0f/16.0f; }

  virtual void ScrollViewBy(int x, int y);
  virtual void UpdateViewport();
  virtual void GetViewMargins(int& left, int& top, int& right, int& bottom);
  virtual void GetOffscreenViewMargins(int& left, int& top, int& right, int& bottom);
  virtual void GetViewSize(float& width, float& height);
...
protected:
   ...
   */
  bool DoFling(const float& aDelta);
   ...
   */
  static void TimeoutTouchListeners(void* data);
...
private:
  ...
  GeckoContentController* mGeckoContentController;
  GestureEventListener* mGestureEventListener;
  ...
  void* mTouchListenerTimeoutTask;
...
//  Monitor mMonitor;
  ...
  nsTArray<long long int> mPreviousPaintDurations;

  // When the last paint request started. Used to determine the duration of
  // previous paints.
  QDateTime mPreviousPaintStartTime;
