 ...
 * used twice.
 */
static const unsigned int MAX_TAP_TIME = 300;
...
GestureEventListener::GestureEventListener(AsyncPanZoomController* aAsyncPanZoomController, mozilla::embedlite::EmbedLiteApp* aApp)
  ...
  case MultiTouchInput::MULTITOUCH_ENTER: {
    ...
    if (length == 1) {
      ...
      if (mState == GESTURE_NONE) {
        ...
        mLongTapTimeoutTask = mApp->PostTask(&GestureEventListener::TimeoutLongTap, this, 500);
      }
    } else if (length == 2) {
  ...
  case MultiTouchInput::MULTITOUCH_LEAVE: {
    ...
    if (event.mTime - mTapStartTime <= MAX_TAP_TIME) {
      if (mState == GESTURE_WAITING_DOUBLE_TAP &&
        ...
        mApp->CancelTask(mDoubleTapTimeoutTask);
        mDoubleTapTimeoutTask = NULL;
        TimeoutDoubleTap(this);
      ...
      if (mState == GESTURE_WAITING_DOUBLE_TAP) {
        ...
        mApp->CancelTask(mLongTapTimeoutTask);
        mLongTapTimeoutTask = NULL;
        ...
        mDoubleTapTimeoutTask = mApp->PostTask(&GestureEventListener::TimeoutDoubleTap, this, MAX_TAP_TIME);
      }
...
void GestureEventListener::TimeoutDoubleTap(void* data)
{
  GestureEventListener* self = static_cast<GestureEventListener*>(data);
  self->mDoubleTapTimeoutTask = nullptr;
  // If we haven't gotten another tap by now, reset the state and treat it as a
  // single tap. It couldn't have been a double tap.
  if (self->mState == GESTURE_WAITING_DOUBLE_TAP) {
    self->mState = GESTURE_NONE;

    self->HandleSingleTapConfirmedEvent(self->mLastTouchInput);
  }
}

void GestureEventListener::TimeoutLongTap(void* data)
{
  GestureEventListener* self = static_cast<GestureEventListener*>(data);
  self->mLongTapTimeoutTask = nullptr;
  // If the tap has not been released, this is a long press.
  if (self->mState == GESTURE_WAITING_SINGLE_TAP) {
    self->mState = GESTURE_NONE;

    self->HandleLongTapEvent(self->mLastTouchInput);
  }
}
