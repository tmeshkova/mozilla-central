 ...
 */
static const int32_t PAN_REPAINT_INTERVAL = getenv("PZ_1") ? atoi(getenv("PZ_1")) : 250;

/**
 * Maximum amount of time flinging before sending a viewport change. This will
 * asynchronously repaint the page.
 */
static const int32_t FLING_REPAINT_INTERVAL = getenv("PZ_2") ? atoi(getenv("PZ_2")) : 75;

/**
 * Minimum amount of speed along an axis before we begin painting far ahead by
 * adjusting the displayport.
 */
static const float MIN_SKATE_SPEED = getenv("PZ_3") ? atof(getenv("PZ_3")) : 0.7f;

/**
 * Duration of a zoom to animation.
 */
static const int ZOOM_TO_DURATION = 250;
 ...
 * requests.
 */
static const int NUM_PAINT_DURATION_SAMPLES = getenv("PZ_4") ? atoi(getenv("PZ_4")) :  3;

AsyncPanZoomController::AsyncPanZoomController(mozilla::embedlite::EmbedLiteApp* aApp, mozilla::embedlite::EmbedLiteView* aView,
...
//     mMonitor("AsyncPanZoomController"),
     mLastSampleTime(QDateTime::currentDateTime()),
     mState(NOTHING),
     mDPI(72),
     ...
     mView(aView)
{
  if (aGestures == USE_GESTURE_DETECTOR) {
    mGestureEventListener = new GestureEventListener(this, mApp);
  }
  mKinetic = new EmbedKineticModule(this, mApp);
  ...
  if (mState == WAITING_LISTENERS || mHandlingTouchQueue) {
    if (aEvent.mInputType == MULTITOUCH_INPUT) {
      ...
      if (!mTouchListenerTimeoutTask) {
        mTouchListenerTimeoutTask = mApp->PostTask(&AsyncPanZoomController::TimeoutTouchListeners, this, TOUCH_LISTENER_TIMEOUT);
      }
    }
...
//        MonitorAutoLock monitor(mMonitor);
    ...
    case NOTHING:
      mX.StartTouch(xPos);
      mY.StartTouch(yPos);
      printf(">>>>>>Func:%s::%d p[%i,%i]\n", __PRETTY_FUNCTION__, __LINE__, xPos, yPos);
      mKinetic->MousePress(xPos, yPos, aEvent.mTime);
      SetState(TOUCHING);
      break;
    case TOUCHING:
    case PANNING:
...
nsEventStatus AsyncPanZoomController::OnTouchMove(const MultiTouchInput& aEvent) {
  ...
  switch (mState) {
    ...
    case TOUCHING: {
      printf(">>>>>>Func:%s::%d TOUCHING time:%i\n", __PRETTY_FUNCTION__, __LINE__, aEvent.mTime);
...
//      MonitorAutoLock monitor(mMonitor);
      SingleTouchData& touch = GetFirstSingleTouch(aEvent);
      printf(">>>>>>Func:%s::%d PANNING[%i,%i]\n", __PRETTY_FUNCTION__, __LINE__, touch.mScreenPoint.x, touch.mScreenPoint.y);
      mKinetic->MouseRelease(touch.mScreenPoint.x, touch.mScreenPoint.y, aEvent.mTime);
      ScheduleComposite();
      RequestContentRepaint();
    }
    mX.EndTouch();
...
void AsyncPanZoomController::UpdateWithTouchAtDevicePoint(const MultiTouchInput& aEvent) {
  ...
  nsIntPoint point = touch.mScreenPoint;
  int32_t xPos = point.x, yPos = point.y;
  int timeDelta = aEvent.mTime - mLastEventTime;
...
//      printf(">>>>>>Func:%s::%d ERR invRes:%g, timeD:%i\n", __PRETTY_FUNCTION__, __LINE__, inverseResolution, timeDelta);
      return;
    }

#if 1
    SingleTouchData& touch = GetFirstSingleTouch(aEvent);
    mKinetic->MouseMove(touch.mScreenPoint.x, touch.mScreenPoint.y, aEvent.mTime);
#else
...
bool AsyncPanZoomController::DoFling(const float& aDelta) {
  if (mState != FLING) {
    return false;
  }
  ...
  double aEstimatedPaintDuration)
{
  ...
  // at any instant.
  const float X_SKATE_SIZE_MULTIPLIER = getenv("AZMX_1") ? atof(getenv("AZMX_1")) : 3.0f;
  const float Y_SKATE_SIZE_MULTIPLIER = getenv("AZMY_1") ? atof(getenv("AZMY_1")) : 3.5f;
  ...
  const float X_STATIONARY_SIZE_MULTIPLIER = getenv("AZMX_2") ? atof(getenv("AZMX_2")) : 1.5f;
  const float Y_STATIONARY_SIZE_MULTIPLIER = getenv("AZMY_2") ? atof(getenv("AZMY_2")) : 2.5f;
...
//  }
    mView->ScheduleRender();
}

void AsyncPanZoomController::RequestContentRepaint() {
  mPreviousPaintStartTime = QDateTime::currentDateTime();

  double estimatedPaintSum = 0.0;
  for (uint32_t i = 0; i < mPreviousPaintDurations.Length(); i++) {
    estimatedPaintSum += mPreviousPaintDurations[i] / 1000;
  }
...
//                      mFrameMetrics));
  mView->UpdateDisplayPort(mFrameMetrics);
...
bool AsyncPanZoomController::SampleContentTransformForFrame(const QDateTime& aSampleTime,
...
void
AsyncPanZoomController::ApplyTransformToView()
{
  float slocalScale = 1.0;
  nsIntPoint scrollCompensation(mFrameMetrics.mScrollOffset.x, mFrameMetrics.mScrollOffset.y);
  mView->SetTransformation(slocalScale, scrollCompensation);
}
...
//  MonitorAutoLock monitor(mMonitor);
  ...
  if (mWaitingForContentToPaint) {
    ...
    mPreviousPaintDurations.AppendElement(
      QDateTime::currentDateTime().toMSecsSinceEpoch() - mPreviousPaintStartTime.toMSecsSinceEpoch());
  } else {
...
//    MonitorAutoLock mon(mMonitor);
    ...
    mAnimationStartTime = QDateTime::currentDateTime();
...
void AsyncPanZoomController::ContentReceivedTouch(bool aPreventDefault) {
  ...
  if (mTouchListenerTimeoutTask) {
    mApp->CancelTask(mTouchListenerTimeoutTask);
    mTouchListenerTimeoutTask = nullptr;
  }
...
void AsyncPanZoomController::TimeoutTouchListeners(void* data) {
  AsyncPanZoomController* self = static_cast<AsyncPanZoomController*>(data);
  self->mTouchListenerTimeoutTask = nullptr;
  self->
...
void AsyncPanZoomController::ScrollViewBy(int x, int y)
{
  ScrollBy(gfx::Point(-x, -y));
  ScheduleComposite();
}

void AsyncPanZoomController::UpdateViewport()
{
  RequestContentRepaint();
}

void AsyncPanZoomController::GetViewMargins(int& left, int& top, int& right, int& bottom)
{
  const FrameMetrics& metrics = mFrameMetrics;
  gfx::Rect cssCompositedRect =
    AsyncPanZoomController::CalculateCompositedRectInCssPixels(metrics);
  left = mFrameMetrics.mScrollOffset.x > 0 ? 0 : -mFrameMetrics.mScrollOffset.x;
  top = mFrameMetrics.mScrollOffset.y > 0 ? 0 : -mFrameMetrics.mScrollOffset.y;
  bottom = mFrameMetrics.mContentRect.height - cssCompositedRect.height - mFrameMetrics.mScrollOffset.y;
  if (bottom > 0) bottom = 0;
  bottom = abs(bottom);
  right = mFrameMetrics.mContentRect.width - cssCompositedRect.width - mFrameMetrics.mScrollOffset.x;
  if (right > 0) right = 0;
  right = abs(right);
}

void AsyncPanZoomController::GetOffscreenViewMargins(int& left, int& top, int& right, int& bottom)
{
  const FrameMetrics& metrics = mFrameMetrics;
  gfx::Rect cssCompositedRect =
    AsyncPanZoomController::CalculateCompositedRectInCssPixels(metrics);
  
  left = mFrameMetrics.mScrollOffset.x > 0 ? mFrameMetrics.mScrollOffset.x : 0;
  top = mFrameMetrics.mScrollOffset.y > 0 ? mFrameMetrics.mScrollOffset.y : 0;
  bottom = mFrameMetrics.mContentRect.height - cssCompositedRect.height - mFrameMetrics.mScrollOffset.y;
  if (bottom < 0) bottom = 0;
  right = mFrameMetrics.mContentRect.width - cssCompositedRect.width - mFrameMetrics.mScrollOffset.x;
  if (right < 0) right = 0;
...
void AsyncPanZoomController::GetViewSize(float& width, float& height)
{
  width = mFrameMetrics.mCompositionBounds.width;
  height = mFrameMetrics.mCompositionBounds.height;
}

void AsyncPanZoomController::GetContentSize(float& width, float& height)
{
  width = mFrameMetrics.mContentRect.width;
  height = mFrameMetrics.mContentRect.height;
}
