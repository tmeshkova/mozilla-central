...
TabChildHelper::ProcessUpdateFrame(const FrameMetrics& aFrameMetrics)
{
  ...
  if (sPostAZPCAsJsonViewport) {
    ...
    data.AppendPrintf(", \"cssPageRect\" : ");
        ...
        data.AppendPrintf("{ \"width\" : %f", aFrameMetrics.mZoom.scale);
        data.AppendPrintf(", \"height\" : %f", aFrameMetrics.mZoom.scale);
        data.AppendPrintf(", \"scale\" : %f", aFrameMetrics.mZoom.scale);
        data.AppendPrintf(" }");
